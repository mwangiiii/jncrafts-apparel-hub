...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const createEmailHtml = (isForAdmin: boolean = false) => `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        ...
        <style>
          ...
          .admin-alert { background-color: #fef3cd; border: 1px solid #f0ad4e; padding: 15px; border-radius: 8px; margin: 20px 0; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="content">
            ${isForAdmin ? `
              <div class="admin-alert">
                ...
                <p>Order #${orderNumber} status has been changed to <strong>${orderStatus.toUpperCase()}</strong></p>
              </div>
            ` : ''}
            
            <h2>${isForAdmin ? `Order Status Update - ${customerName}` : statusInfo.title}</h2>
            <div class="status-badge">${orderStatus.toUpperCase()}</div>
            
            <p>${isForAdmin ? `Customer ${customerName}'s order has been updated to ${orderStatus}.` : statusInfo.message}</p>
            
            <div class="order-details">
              <h3>Order Details</h3>
              <p><strong>Order Number:</strong> ${orderNumber}</p>
              <p><strong>Customer:</strong> ${customerName}</p>
              ${isForAdmin ? `<p><strong>Customer Email:</strong> ${customerEmail}</p>` : ''}
            ...
            <p>${isForAdmin ? 'This is an automated notification of the order status change.' : 'If you have any questions about your order, please contact our customer service team.'}</p>
          </div>
    ...
    const adminEmailResponse = await resend.emails.send({
      from: "jnCrafts <onboarding@resend.dev>",
      to: [adminEmail],
      subject: `Order Status Update - #${orderNumber} - ${orderStatus.toUpperCase()}`,
