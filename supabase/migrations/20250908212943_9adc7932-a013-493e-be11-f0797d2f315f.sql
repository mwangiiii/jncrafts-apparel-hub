...
-- Note: Removing CONCURRENTLY since migrations run inside transactions
...
CREATE INDEX IF NOT EXISTS idx_product_colors_product_id_active 
ON product_colors(product_id) WHERE is_available = true;

CREATE INDEX IF NOT EXISTS idx_product_sizes_product_id_active 
ON product_sizes(product_id) WHERE is_available = true;

CREATE INDEX IF NOT EXISTS idx_colors_active_display_order 
ON colors(id, display_order) WHERE is_active = true;

CREATE INDEX IF NOT EXISTS idx_sizes_active_display_order 
ON sizes(id, display_order) WHERE is_active = true;

-- Optimized get_product_complete function with CTEs for better performance
