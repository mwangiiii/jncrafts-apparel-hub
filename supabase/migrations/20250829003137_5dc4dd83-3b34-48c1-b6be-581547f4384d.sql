-- Fix security definer view issue by recreating views with security_invoker=on
...
-- Check for any other views that need fixing and recreate materialized view properly
DROP MATERIALIZED VIEW IF EXISTS public.mv_products_landing CASCADE;

CREATE MATERIALIZED VIEW public.mv_products_landing 
WITH (security_invoker=on) AS
SELECT 
    p.id,
    p.name,
...
COMMENT ON MATERIALIZED VIEW public.mv_products_landing IS 'Optimized materialized view for product listings with security_invoker enabled.';