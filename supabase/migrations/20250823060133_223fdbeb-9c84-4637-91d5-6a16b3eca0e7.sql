...
BEGIN
  ...
  -- Return valid discount (excluding sensitive fields)
  RETURN jsonb_build_object(
    'valid', true,
...
-- Allow public access only to banner messages for display (not codes)
...
-- Add RLS policy for the new validation function usage
COMMENT ON FUNCTION public.validate_discount_code(text, numeric) IS 'Securely validates discount codes without exposing all codes to clients';